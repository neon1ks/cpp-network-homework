...
int print_ips_with_getaddrinfo(const std::string &host_name)
{
    ...
    const int status = getaddrinfo(host_name.c_str(), nullptr, &hints, &servinfo);
    ...
    for (auto const *s = servinfo; s != nullptr; s = s->ai_next)
    {
        ...
        if (AF_INET == s->ai_family)
        {
            ...
            std::cout << "AF_INET\n";
            auto const* const sin = reinterpret_cast<const sockaddr_in* const>(s->ai_addr);
        ...
        else if (AF_INET6 == s->ai_family)
        {
            ...
            std::cout << "AF_INET6\n";
            auto const* const sin = reinterpret_cast<const sockaddr_in6* const>(s->ai_addr);
...
int print_ips_with_gethostbyname(const std::string &host_name)
{
    ...
    socket_wrapper::SocketWrapper const sock_wrap;
    const hostent *remote_host { gethostbyname(host_name.c_str()) };

    if (nullptr == remote_host)
    {
        if (sock_wrap.get_last_error_code() != 0)
        {
    ...
    for (const char* const* p_alias = const_cast<const char* const*>(remote_host->h_aliases); *p_alias != nullptr; ++p_alias)
    {
    ...
    if (AF_INET == remote_host->h_addrtype)
    {
        ...
        for (int i = 0; remote_host->h_addr_list[i] != nullptr; ++i)
        {
            addr.s_addr = *reinterpret_cast<const in_addr_t* const>(remote_host->h_addr_list[i]);
